...
class UrlMatchingGoalQuery(object):
    ...
    def get_hits_from_site_query(self, index, pattern_type, field, value):
        ...
        if str(pattern_type) == "contains": 
            body = self.get_query_contain_type_body(field, value, self.from_time, self.end_time)

        elif str(pattern_type) == "regex":
            body = self.get_query_regex_type_body(field,value, self.from_time, self.end_time)
    ...
    def enter_query(self):
        ...
        for element_data in self.data:
            ...
            data = self.get_hits_from_site_query(self.index_site_tracking,element_data.pattern_type, field, value)
